...
def like_post(request):
    username = request.user.username #достаємо ім'я поточного юзера щоб знати хто лайкає пост і чи не лайка він його раніше 
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                # Логінимо користуваче і редіректимо його
                user_login = auth.authenticate(username=username, password=password)
                auth.login(request, user_login)

                # Робимо об'єкт класа Профіль і створюєм користувача
